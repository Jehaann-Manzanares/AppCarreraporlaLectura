...
{if((xmlDoc.getElementsByTagName("sensible_mayusculas")[0].childNodes[0]!=undefined)&&(xmlDoc.getElementsByTagName("sensible_mayusculas")[0].childNodes[0]!=null))sensible_mayusculas=xmlDoc.getElementsByTagName("sensible_mayusculas")[0].childNodes[0].nodeValue;else sensible_mayusculas="no";if((xmlDoc.getElementsByTagName("sensible_acentos")[0].childNodes[0]!=undefined)&&(xmlDoc.getElementsByTagName("sensible_acentos")[0].childNodes[0]!=null))sensible_acentos=xmlDoc.getElementsByTagName("sensible_acentos")[0].childNodes[0].nodeValue;else sensible_acentos="no";if(xmlDoc.getElementsByTagName("sensible_mayusculas_ocultar")[0].childNodes[0]!=undefined)
...
{$('#btnComprobar').text(comprobar);$('#btnComprobar').click(function(e){e.preventDefault();compruebaTexto();});};function inicializarVariablesXapi(){}
...
{$('#btnComprobar').unbind("click");for(k=0;k<pa.length;k++)
...
if(contenido!=undefined){contenido=contenido.toLowerCase()}
var cadenaIntentosXAPI=$('#numIntentos').text();var posXAPI=cadenaIntentosXAPI.indexOf("/");var numeroIntentosXAPI=parseInt(cadenaIntentosXAPI.substring(0,posXAPI+1));if(respuestaF.toLowerCase()==contenido){exito=1}
else{exito=0}
if(contenido==undefined){contenido=""}
responderEscritaXapi(respuestaF.toLowerCase(),respuestaF.toLowerCase(),contenido,exito)}
completarXapi("KO",puntosReg,tiempoReg);cargarPantallaFinal('intentos');}}
...
if(contenido!=undefined){contenido=contenido.toLowerCase()}
var cadenaIntentosXAPI=$('#numIntentos').text();var posXAPI=cadenaIntentosXAPI.indexOf("/");var numeroIntentosXAPI=parseInt(cadenaIntentosXAPI.substring(0,posXAPI+1));if(numeroIntentosXAPI+1<=numero_intentos){if(respuestaF.toLowerCase()==contenido){exito=1}
else{exito=0}}}
if(error==1)
{if(op!="finIntentos")
{if(op!="finTiempo")
...
if(contenido==undefined){contenido=""}
responderEscritaXapi(respuestaF.toLowerCase(),respuestaF.toLowerCase(),contenido,exito)}
completarXapi("OK",puntosReg,tiempoReg);cargarPantallaFinal('OK');}}
